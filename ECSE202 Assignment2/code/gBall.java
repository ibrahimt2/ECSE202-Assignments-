...
public class gBall extends Thread {
	...
	private static final int HEIGHT = 800; 		// n.b. screen coords
	...
	public gBall(double Xi, double Yi, double bSize, Color bColor, double bLoss, double bVel) { //Constructor for the gBall
		...
		this.bSize = bSize * SCALE_FACTOR;	//Sets radius of ball, and the 6 adjusts for metres
		this.bColor = bColor;				//Sets color of ball
		this.bLoss = bLoss;					//Sets energy loss of ball
		this.bVel = bVel;					//Sets horizontal velocity
	...
	public void run () {  							//The run part of the thread. Contains all of the physics of the ball
		...
		while (vy > 0.05) { 					//Stops the while loop if the vertical velocity of the ball drops below a 0.05, which is essentially motionless
			...
			if (h >= 0) {
			BouncingBall.setLocation(xPos, HEIGHT - OFFSET - h - bSize); //Changes the location of the balls when they are in the air
			}
